...
# --- Document authoring (doc) graph prompts ---
# 분류: 작성 요청이 어떤 문서 종류(프리셋)인지 id 하나로 판별.
DOC_CLASSIFY_PROMPT = """다음 작성 요청이 어떤 문서 종류인지 분류하라.
...
# 초안 생성: id 태깅된 사내 문서 컨텍스트를 근거로 canvas 트리(JSON 객체)를 출력.
# 섹션(조항 등) → 블록(문단/목록/표/서명란 등) 계층. 블록마다 실제로 사용한
# chunk_id만 source_refs에 인용(없으면 빈 배열). section_id/block_id는 서버가 부여.
DOC_DRAFT_PROMPT = """너는 사내 문서를 근거로 정형 문서(계약서·기안서 등)의 초안을 작성하는 전문 작성자다.
...
- 블록은 문단(paragraph)·항목(list_item)·표(table)·미정값(placeholder)·서명란(signature_field)
  중 하나의 block_type을 가진다. 항/호는 list_item으로 둔다.
- 금액·날짜·기간·지분·지역 등 아직 정해지지 않은 값은 본문 안에 반드시 `{{{{항목명}}}}` 토큰으로
  써라(예: "계약기간은 {{{{계약 기간}}}}으로 한다"). 대괄호 빈칸([ ])은 쓰지 말 것. 이 `{{{{항목명}}}}`의
  항목명은 아래 missing_terms의 label과 **정확히 동일한 문자열**이어야 한다(클라이언트가 이 토큰을
  치환해 값을 채운다).
...
# 블록 단위 수정: 현재 블록 본문 + 전체 문서 맥락 + 사용자 피드백 → 수정된 본문만 출력.
DOC_BLOCK_EDIT_PROMPT = """너는 문서의 특정 블록만 수정하는 편집자다.
...
# 전체 재작성: 기존 draft 프롬프트 뒤에 덧붙는 피드백 헤더(사용자 피드백은 별도로 이어붙임).
# 원 요청·참고 컨텍스트는 draft 프롬프트가 이미 담고 있고, 여기에 "이전 초안의 무엇이
# 마음에 안 들었는지"를 얹어 처음부터 다시 쓰게 한다.
DOC_REGENERATE_PROMPT = """[전체 재작성 요청]
사용자가 이전 초안이 마음에 들지 않아 재작성을 요청했다. 위의 원 요청과 참고 컨텍스트를
그대로 근거로 삼되, 아래 피드백을 반영해 문서 전체를 처음부터 다시 작성하라. 이전 초안의
구조·표현에 얽매이지 말고, 피드백에서 지적된 점을 확실히 개선하라.
